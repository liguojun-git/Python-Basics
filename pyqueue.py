...
# 常用方法
# 1. put(item, block=True, timeout=None)        将 item 放入队列。如果 block 为 True 且队列已满，则等待 timeout 秒，直到队列有空闲空间。如果 timeout 为 None，则无限等待。
# 2. get(block=True, timeout=None)              从队列中获取并移除一个元素。如果 block 为 True 且队列为空，则等待 timeout 秒，直到队列中有元素。如果 timeout 为 None，则无限等待。
# 3. qsize()                                    返回队列中的元素数量。
# 4. empty()                                    如果队列为空，返回 True，否则返回 False。
# 5. full()                                     如果队列已满，返回 True，否则返回 False。
