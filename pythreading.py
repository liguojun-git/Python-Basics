...
# 为什么使用多线程？
# 在单线程程序中，任务是一个接一个地顺序执行的。如果某个任务需要等待（例如等待网络响应或文件读取），整个程序会被阻塞，直到该任务完成。
# 而多线程可以让程序在等待某个任务的同时，继续执行其他任务，从而提高程序的整体性能。
...
# 那么，Python 多线程的用武之地在哪里？ 对于 I/O 密集型任务（如网络请求、读写文件、等待用户输入），线程在等待 I/O 操作完成时会释放 GIL，从而让其他线程运行。
# 这可以显著提升程序的整体响应速度和效率，因为你在等待一个网页响应时，程序可以去处理另一个任务。
...
def worker(q):
    while not q.empty():
        ...
        print(f"[{thread_name}] 处理项目: {item}")  # ← 加上 thread_name
        print(f"处理项目: {item}")
        q.task_done()
