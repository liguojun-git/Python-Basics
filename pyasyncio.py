...
# 想象一下你正在经营一家餐厅：
# 同步模式（普通函数）： 你只有一个厨师。客人 A 点了一份牛排，厨师开始煎牛排（这需要等待 5 分钟）。在煎牛排的这 5 分钟里，厨师完全被占用，不能做任何其他事，
# 即使客人 B 只想点一杯水，也必须干等着。
# 异步模式（asyncio）： 你有多个厨师（实际上还是一个，但非常聪明）。厨师开始煎客人 A 的牛排后，发现需要等待，他立刻把这份牛排标记为等待中，然后转头去给客人 B 倒水。
# 倒完水回来，看看牛排是不是快好了，如果还没好，又可以去处理客人 C 的订单。这样，在等待 I/O（如煎牛排、网络请求、读写文件）的时间里，厨师（CPU）一直在高效地工作。
...
#                                                                                                                               return_exceptions: 默认为 False，任何任务抛出异常都会立即传播给 gather 的调用者。
#                                                                                                                               设为 True 时，异常会作为结果的一部分返回。
# asyncio.sleep(delay, result=None)	            异步地休眠指定秒数。这是与 time.sleep（阻塞）的关键区别。	                        delay: 休眠的秒数。
#                                                                                                                               result: 休眠结束后返回的值。
# asyncio.wait(aws, *, timeout=None,    并发运行任务，并等待满足指定条件。返回两个集合 (done, pending)，分别是已完成和未完成的任务。   aws: 异步对象集合。
# return_when=ALL_COMPLETED)		                                                                                             timeout: 超时时间（秒）。
#
#                                                                                                                               return_when: 决定何时返回，可选：
#                                                                                                                               FIRST_COMPLETED（第一个完成）、
#                                                                                                                               FIRST_EXCEPTION（第一个异常）、
#                                                                                                                               ALL_COMPLETED（全部完成，默认）。
# asyncio.to_thread(func, /, *args, **kwargs)	（Python 3.9+）将一个普通的、可能阻塞的同步函数放到一个单独的线程中运行，               func: 要在线程中运行的同步函数。
#                                                   并返回一个可 await 的协程。用于处理 CPU 密集型或阻塞式 I/O。	                *args, **kwargs: 传递给函数的参数。



# 它维护一个任务队列，当一个任务执行到 await（例如等待网络响应）时，它会被挂起，事件循环立即从队列中找出下一个可以运行（就绪）的任务来执行。
# 当被挂起任务的 I/O 操作完成后，事件循环会收到通知，将该任务状态改回就绪，
# 并在未来某个时刻继续执行它。通过这种方式，在 I/O 等待期间，CPU 被充分利用来执行其他任务，实现了单线程内的并发。
