...
# 默认情况下，logging 模块只显示 WARNING 及以上级别的日志。
...
""" logging.basicConfig(
    ...
    # filename="app.log",
    handlers=[
        logging.FileHandler("app.log", encoding='utf-8')  # 指定 UTF-8 编码
    ]
) """
...
# 3. Handler 常用类型
# Handler 类型	                                说明	                                         示例
# StreamHandler	                        输出到流（如控制台）	                        andler = logging.StreamHandler()
...
""" # 创建父 logger（根 logger）
root_logger = logging.getLogger()  # 根 logger
root_logger.setLevel(logging.WARNING)  # 父级设置为 WARNING

# 创建子 logger
child_logger = logging.getLogger("child")  # 子 logger
child_logger.setLevel(logging.NOTSET)  # NOTSET = 听父级的

# 测试
child_logger.debug("调试信息")   # ❌ 不输出（父级是 WARNING）
child_logger.info("普通信息")     # ❌ 不输出
child_logger.warning("警告信息")  # ✅ 输出（达到 WARNING）
child_logger.error("错误信息")    # ✅ 输出 """
