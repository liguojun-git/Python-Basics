...
# 为什么使用 subprocess 模块？
# 在 Python 中，有时我们需要执行一些系统命令或调用其他程序来完成特定的任务。例如，你可能需要运行一个 shell 命令、启动一个外部应用程序，
# 或者与一个命令行工具进行交互。subprocess 模块提供了一种安全且灵活的方式来处理这些需求。
...
try:
    ...
    result = subprocess.run(['dir', 'nonexistent_file'], capture_output=True, text=True, shell=True,check=True)
...
# Windows: 使用 dir 替代 ls，findstr 替代 grep
p1 = subprocess.Popen(
    ['cmd', '/c', 'dir'],  # Windows 的 ls 替代
    stdout=subprocess.PIPE,
    text=True
)

p2 = subprocess.Popen(
    ['findstr', 'py'],  # Windows 的 grep 替代
    stdin=p1.stdout,
    stdout=subprocess.PIPE,
    text=True,
    shell=True  # findstr 需要 shell
...
""" try:
 ...
 """
try:
    # Windows 使用 timeout 命令（单位：秒）
    # /t 指定时间，/nobreak 忽略按键中断
    subprocess.run(["timeout", "/t", "5", "/nobreak"], timeout=3)
except subprocess.TimeoutExpired:
    print("命令超时！")