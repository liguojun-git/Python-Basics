...
# 这是一个 Scrapy 命令，用于启动爬虫并将抓取的数据保存到文件中。
# scrapy crawl douban_spider -o douban_movies.csv
# scrapy	调用 Scrapy 框架
# crawl	Scrapy 的命令之一，表示"运行爬虫"
# douban_spider	爬虫的名称（你在 name = "douban_spider" 中定义的那个）
# -o	输出选项（output）的简写，表示"保存结果到文件"
# douban_movies.csv	输出文件的名称，扩展名 .csv 表示保存为 CSV 格式
