...
# 第二步：从文件读取XML
parsed_tree = ET.parse('books.xml')  # 解析文件
parsed_root = parsed_tree.getroot()  # 获取根元素
...
class MovieHandler( xml.sax.ContentHandler ):
   def __init__(self):
      self.CurrentData = ""    # 当前正在处理的元素名
      self.type = ""           # 存储 type 元素的内容
      self.format = ""         # 存储 format 元素的内容
      self.year = ""           # 存储 year 元素的内容
      self.rating = ""         # 存储 rating 元素的内容
      self.stars = ""          # 存储 stars 元素的内容
      self.description = ""    # 存储 description 元素的内容
